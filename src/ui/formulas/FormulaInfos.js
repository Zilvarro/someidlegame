...
export default function FormulaInfos() {
  ...
  const basicResetsUnlocked = layerFinished || stageRun.currentStage > 0 || stageRun.formulaUnlockCount >= 4
  const xResetStatus = context.game.validate("xReset")
  const xResetAvailable = xResetStatus.valid
  const differentialTarget = stageDictionary[stageList[stageRun.currentStage]].xGoal
  ...
  return (<div>
    ...
    <InfoLine visible={!basicResetsUnlocked}>Unlock {4 - stageRun.formulaUnlockCount} more formula{stageRun.formulaUnlockCount !== 3 && "s"} to enable Basic Resets</InfoLine>
    ...
    <InfoLine visible={basicResetsUnlocked && differentialTarget && !xResetAvailable}>Reach x={formatNumber(differentialTarget, context.settings.numberFormat)} for the next x-Reset</InfoLine>
    <div style={{color:"#00FF00", fontWeight:"bold"}}><InfoLine visible={!layerFinished && xResetAvailable}>{sResetName}-Reset is now available! (See button above!)</InfoLine></div>
...
// {(state.progressionLayer > 0 || state.highestXTier > 0) && state.autoUnlockIndex < shopFormulas.length && state.xValues[0] < nextUnlockCost && (nextUnlockCost <= alphaTarget || state.progressionLayer > 0) && <p>Next Formula at x={formatNumber(nextUnlockCost, state.settings.numberFormat)}</p>}
// {(state.progressionLayer > 0 || state.highestXTier > 0) && state.autoUnlockIndex < shopFormulas.length && state.xValues[0] >= nextUnlockCost && <p>New Formula available</p>}
...
// <p></p>
// {state.progressionLayer === 0 && (state.xValues[0] >= alphaTarget ?
...
// {hashtagB && <>#B = {formatNumber(state.basicResetCount, state.settings.numberFormat, 3)}&nbsp;&nbsp;(Basic Resets)<br/></>}
